...
# data = data.sort_values(by=ins_index)
y_indexs = ["volume"]
...
# model._instantiate_model(training)
# cat_columns = data.select_dtypes(["category"]).columns
# data[cat_columns] = data[cat_columns].apply(lambda x: x.cat.codes)
# data.rename(columns={"time_idx": "timepoints"}, inplace=True)
ins_index = ["agency", "sku", "time_idx"]
# data = data.sort_values(by=ins_index)
y_indexs = ["volume"]
x_indexs = data.columns.drop(y_indexs + ins_index)
...
def _predictions_to_dataframe(predictions):
    ...
    data = index.loc[index.index.repeat(6)].reset_index(drop=True)
    data = data.reindex(columns=columns_names)
    data["volume"] = output.flatten()
    for i in range(output.shape[0]):
        start_idx = i * 6
        start_time = data.loc[start_idx, time_idx]
        data.loc[start_idx : start_idx + 6 - 1, time_idx] = np.arange(
...
res = _predictions_to_dataframe(predictions)

exit()
print(pd.DataFrame({"volume": predictions.output.cpu().numpy()}))
